    ...
    function handleSubmit() {
      ...
      .then((res) => {
          if (res.data!=null) {
              ...
              // Handle the case when no flights are available
...
<table style={{ color: "blue", border: "1px solid white", width: "100%", borderCollapse: "collapse",margin:"5vw" }}>
  <thead style={{ backgroundColor: "black", color: "white" }}>
    <tr>
      ...
      <th style={{ border: "1px solid white", padding: "8px", color: "rgb(251, 87, 28)" }}>Book</th>
    </tr>
  </thead>
  <tbody>
    {responseData !== null &&
      responseData.map((user) => (
        <tr key={user._id}>
          ...
          <td>
            <button style={{ backgroundColor: "transparent", borderRadius: "30px", color: "white" }} ref1={user.num}>
              Book
            </button>
          </td>
        </tr>
